 ...
 * @param {Array} chatHistory - Array of chat entries (recent chats)
 * @param {string} userId - The user ID to fetch existing summary
 ...
 */
async function generateChatSummary(chatHistory, userId, maxLength = 800) {
  if (!chatHistory || chatHistory.length === 0) {
    return "No previous conversation history.";
  }

  try {
    // First, try to get the existing chat summary
    let existingSummary = "";
    try {
      const chatSummaryRef = db.collection('chatSummary').doc(userId);
      const chatSummaryDoc = await chatSummaryRef.get();
      
      if (chatSummaryDoc.exists && chatSummaryDoc.data().summary) {
        existingSummary = chatSummaryDoc.data().summary;
        console.log(`Retrieved existing chat summary for user ${userId}`);
      }
    } catch (error) {
      console.error('Error retrieving existing chat summary:', error);
      // Continue without existing summary if there's an error
    }

    // Prepare the recent chat history in a format suitable for GPT
    ...
    for (const chat of sortedChats) {
      recentConversationText += `User: ${chat.question}\nLaura: ${chat.response}\n\n`;
    }
    
    // Use GPT to generate an updated summary based on existing summary and recent chats
      ...
      messages: [
        {
          ...
          Your task is to update an existing conversation summary with new conversation entries.
          
          IMPORTANT: Your updated summary MUST include the following key information when mentioned in the conversation:
        ...
        {
          role: 'user',
          content: `Here is the existing conversation summary:\n\n${existingSummary || "No previous summary available."}\n\nHere are the recent conversation entries to incorporate:\n\n${recentConversationText}\n\nPlease create an updated comprehensive summary that incorporates both the existing summary and the new conversation entries. If there are conflicts between old and new information, prioritize the newer information.`
        }
      ],
      temperature: 0.5,
    ...
    console.log(`Generated updated comprehensive GPT summary incorporating existing summary and recent chats`);
    return summary;
  } catch (error) {
